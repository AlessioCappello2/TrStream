...
def normalize_revolut(table: pa.Table) -> pa.Table:
    """
    Normalize and filter revolut data contained in a given Arrow table.
    
    :param table: input revolut table
    :type table: pa.Table
    ...
    for i, event in enumerate(payloads):
        ...
        rows.append({
            ...
            "user_id": obj["legs"][0]["account_id"],
            "amount": float(obj["legs"][1]["amount"]),
            "currency": obj["legs"][0]["currency"],
            "transaction_type": obj["type"]
        }) 

    if not rows:
        return pa.Table.from_batches([], schema=REVOLUT_SCHEMA)

    return pa.Table.from_pylist(rows, schema=REVOLUT_SCHEMA)